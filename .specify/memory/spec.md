...
- The `body::before` multi-layer background must not create z-index or compositing conflicts with foreground glass cards.
...
- The `body::before` pseudo-element supports a third CSS background layer without compatibility issues (multi-layer backgrounds are universally supported in modern browsers).
