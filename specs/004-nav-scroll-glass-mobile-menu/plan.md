...
| -------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Desktop nav breakpoint `@media (min-width: 769px)` uses a hardcoded literal `769px` instead of a CSS custom property | CSS custom properties (e.g., `var(--lg-layout-breakpoint-tablet)`) cannot be used inside `@media` query conditions in any of the target browsers (Chrome 115+, Firefox 110+, Safari 17.4+); this is a CSS Level 5 feature not yet shipped | The existing token `--lg-layout-breakpoint-tablet: 768px` is declared in `:root` for documentation and JS access only. Using a `calc()` or `env()` workaround would add complexity without browser support. The hardcoded literal `769px` is the only compliant option for the current target browser set. |
